...
const AlbumsPage = () => {
  ...
  useEffect(() => {
    ...
    api.getRecommendedRecords(id).then((albums: Albums[]) => setAlbums(albums))
          ...
          <Card key={album.id + Date.now()} className="rounded-md border-none shadow-sm">
            ...
            <CardFooter className="border border-t-0 rounded-bl-md rounded-br-md p-0">
              ...
              >
                ...
                <span className="text-sm font-semibold">{album.artists.join(', ')}</span> {/* Используйте album.artists.join */}
              </Link>
            </CardFooter>
          </Card>
        ))}
